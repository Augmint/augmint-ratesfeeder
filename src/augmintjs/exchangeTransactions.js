module.exports = {
    ...
    *********************************************************************************/
    ...
    /* Internal fx-es, primarily exported for tests */
    isOrderBetter,
...
async function getMatchingOrders(web3, exchangeInstance, bn_ethFiatRate, gasLimit) {
    const orderBook = await fetchOrderBook(web3, exchangeInstance);
    const matches = calculateMatchingOrders(orderBook.buyOrders, orderBook.sellOrders, bn_ethFiatRate, gasLimit);

    return matches;
}

async function fetchOrderBook(web3, exchangeInstance) {
    ...
    const orderCounts = await exchangeInstance.methods.getActiveOrderCounts().call({ gas: 4000000 });
    ...
    for (let i = 0; i < queryCount; i++) {
        const fetchedOrders = isLegacyExchangeContract
            ? await getOrders(web3, exchangeInstance, constants.TOKEN_BUY, i * chunkSize)
            : await getOrders(web3, exchangeInstance, constants.TOKEN_BUY, i * chunkSize, chunkSize);
    ...
    for (let i = 0; i < queryCount; i++) {
        const fetchedOrders = isLegacyExchangeContract
            ? await getOrders(web3, exchangeInstance, constants.TOKEN_SELL, i * chunkSize)
            : await getOrders(web3, exchangeInstance, constants.TOKEN_SELL, i * chunkSize, chunkSize);
...
async function getOrders(web3, exchangeInstance, orderDirection, offset) {
...
async function matchMultipleOrdersTx(exchangeInstance, buyIds, sellIds) {
...
calculateMatchingOrders(_buyOrders, _sellOrders, bn_ethFiatRate, gasLimit)
    ...
    returns: pairs of matching order id , ordered by execution sequence
        { buyIds: [], sellIds: [], gasEstimate }
*********************************************************************************/
function calculateMatchingOrders(_buyOrders, _sellOrders, bn_ethFiatRate, gasLimit) {
    const sellIds = [];
    const buyIds = [];
