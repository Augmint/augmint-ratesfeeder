...
const log = require("src/augmintjs/helpers/log.js")("DelegatedTransferProcessor");
...
class DelegatedTransferProcessor extends EventEmitter {
    constructor(ethereumConnection) {
        ...
        this.account = null;
        this.parseAccount = null;

        setExitHandler(this._exit.bind(this), "DelegatedTransferProcessor");
    }

    async init() {
        this.account = process.env.DTP_ETHEREUM_ACCOUNT;
        this.parseAccount = {
            user: process.env.DTP_PARSE_USER,
            pass: process.env.DTP_PARSE_PASS
        };

        if (!this.web3.utils.isAddress(this.account)) {
            throw new Error("Invalid DTP_ETHEREUM_ACCOUNT: " + this.account);
        }
        ...
        log.info(
            ...
            DTP_ETHEREUM_ACCOUNT: ${process.env.DTP_ETHEREUM_ACCOUNT}
            DTP_ETHEREUM_PRIVATE_KEY: ${
                process.env.DTP_ETHEREUM_PRIVATE_KEY ? "[secret]" : "not provided"
                }
    ...
    async _checkAndMatchOrders() {
        ...
        if (matchingOrders.buyIds.length > 0) {
            ...
            const tx = await this.exchange.signAndSendMatchMultipleOrders(
                this.account,
                process.env.DTP_ETHEREUM_PRIVATE_KEY,
                matchingOrders
            );
